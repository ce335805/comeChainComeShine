...
def saveGfErrorForL(etaNonNorm, systemSize):
    ...
    #gfAna1 = greenAna1st.anaGreenVecWGreater(kVec, wVec, eta, damping) + greenAna1st.anaGreenVecWLesser(kVec, wVec, eta, damping)
    print("gsNum1L.shape = {}".format(gfNumInf.shape))

    #gfDiff1 = np.abs(np.imag(gfNumInf - gfAna1))
    gfDiff2 = np.abs(np.imag(gfNumInf - gfAna2))

    file = h5py.File("data/gfErrL" + str(prms.chainLength) + ".h5", 'w')
